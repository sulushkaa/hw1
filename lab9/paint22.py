...
def equilateral_triangle(screen):
    ...
    pygame.draw.polygon(screen, colors[current_color], points, width=3)

def right_triangle(screen):
    points = [(x, y), (x, y2), (x2, y2)]
    pygame.draw.polygon(screen, colors[current_color], points, width=3)

def rhombus(screen):
    cx = (x + x2) / 2
    cy = (y + y2) / 2
    half_width = abs(x2 - x) / 2
    half_height = abs(y2 - y) / 2
    points = [(cx, y), (x2, cy), (cx, y2), (x, cy)]
    pygame.draw.polygon(screen, colors[current_color], points, width=3)
